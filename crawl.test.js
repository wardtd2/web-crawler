...
test('Retrives a single URL from the provided html', () => {
    ...
    expect(getURLsFromHTML(html, baseURL)).toBe(expected);
});
...
test('Converts relative URL to absolute', () => {
    ...
    const baseURL = 'blog.boot.dev'
    const expected = ['blog.boot.dev/games/path/'];
    expect(getURLsFromHTML(html, baseURL)).toBe(expected);
});
...
test('Retrieves multiple URLs from HTML', () => {
    ...
    const baseURL = 'blog.boot.dev'
    const expected = ['blog.boot.dev/games/path/', 'blog.boot.dev/games/patha/', 'http://blog.boot.dev/path', 'https://blog.boot.dev/path/banana'];
    expect(getURLsFromHTML(html, baseURL)).toBe(expected);
});